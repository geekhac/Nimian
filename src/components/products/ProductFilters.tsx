    ...
    <div className="bg-white rounded-lg shadow p-6">
      ...
      <div className="mb-6">
        ...
        <div className="space-y-2 max-h-[calc(100vh-300px)] overflow-y-auto pr-2">
          ...
          {(() => {
            const importedBrands = brands.filter((b: any) => b.isImported);
            const domesticBrands = brands.filter((b: any) => !b.isImported);

            return (
              <>
                {importedBrands.length > 0 && (
                  <div className="mb-2">
                    <div className="text-xs text-gray-500 mb-1">进口品牌</div>
                    <div className="space-y-2">
                      {importedBrands.map((brand: any) => (
                        <FilterLink
                          key={brand.id}
                ...
                {domesticBrands.length > 0 && (
                  <div>
                    <div className="text-xs text-gray-500 mb-1">国产品牌</div>
                    <div className="space-y-2">
                      {domesticBrands.map((brand: any) => (
                        <FilterLink
                          key={brand.id}
                          href={`/products?brand_id=${brand.id}`}
                          className={`flex items-center gap-2 cursor-pointer text-sm ${
