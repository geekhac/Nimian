...
// 获取商品总数（用于分页）
async function getProductsCount(searchParams: Promise<SearchParams> | SearchParams) {
  const supabase = await createClient()
  const params = await searchParams
...
// 主页面组件
export default async function ProductsPage({ searchParams }: { searchParams: Promise<SearchParams> | SearchParams }) {
  // ✅ 先解析 searchParams
  const params = await searchParams
            ...
            <div className="bg-white rounded-lg shadow p-4 mb-6">
              <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
               <div className="text-sm text-gray-600">
                {/* 将长文本拆分成更简单的结构 */}
                <span>
                    显示第 {(currentPage - 1) * pageSize + 1} - {Math.min(currentPage * pageSize, totalCount)} 个商品
                </span>
